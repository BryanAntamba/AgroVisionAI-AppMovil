...
class _PanelAgricultorState extends State<PanelAgricultor>
    ...
    try {
      ...
      setState(() {
        _dispositivoConectado =
            prefs.getString('dispositivoConectado') == 'true';
        _dispositivoDesconectado =
  ...
  void _reconectarDispositivo() {
    ...
    Future.delayed(const Duration(seconds: 2), () async {
      ...
      if (_intentosReconexion < _datos.reconexion.intentosParaExito) {
        setState(() {
          _errorReconexion =
              'No se pudo conectar el dispositivo. Inténtelo de nuevo.';
        });
        return;
      }
  ...
  void _iniciarSimulacionCapturas() {
    _detenerSimulacionCapturas();
    _intervaloCaptura = Timer.periodic(
      Duration(milliseconds: _datos.captura.intervaloNuevaCapturaMs),
      (timer) {
            ...
            FadeTransition(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: _dispositivoDesconectado
                      ? const Color(0xFFB56C07)
                      : PanelAgricultorStyles.primaryGreen,
                ),
              ),
            ),
            ...
            InkWell(
              ...
              child: Container(
                ...
                decoration: PanelAgricultorStyles.btnGuardarDecoration.copyWith(
                  ...
                  gradient: _dispositivoDesconectado
                      ? null
                      : const LinearGradient(
                          colors: [
                            PanelAgricultorStyles.darkGreen,
                            PanelAgricultorStyles.primaryGreen,
                          ],
                        ),
                ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Divider(
            color: PanelAgricultorStyles.borderGrey,
            height: 1,
            thickness: 1,
          ),
          const SizedBox(height: 14),
              ...
              child: const Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      'La imagen se capturará automáticamente con la cámara del dispositivo',
                      textAlign: TextAlign.center,
                      style: TextStyle(color: Color(0xFF8FA69C), fontSize: 12),
                    ),
                  ),
                  SizedBox(height: 4),
                  Text(
  ...
  Widget _buildSensores() {
    ...
    final tempOk =
        s.temperaturaAireC >= s.temperaturaOptimaMin &&
        s.temperaturaAireC <= s.temperaturaOptimaMax;
    final humOk =
        s.humedadAirePct >= s.humedadAireOptimaMin &&
        s.humedadAirePct <= s.humedadAireOptimaMax;
    final sueloOk = s.humedadSueloPct >= s.riegoMinimo;
    final luzOk =
        s.intensidadLuzLux >= s.luzOptimaMin &&
        s.intensidadLuzLux <= s.luzOptimaMax;
    final hojaBaja = c.humedadHojaPct < c.humedadHojaOptimaMin;

    final tempPct =
        ((s.temperaturaAireC - s.temperaturaSensorMin) /
                (s.temperaturaSensorMax - s.temperaturaSensorMin))
            .clamp(0.0, 1.0);
    ...
    return _buildSeccion(
      ...
      LayoutBuilder(
        builder: (context, constraints) {
          ...
          Widget buildSizedCard(Widget card) =>
              ...
              buildSizedCard(
                _buildSensorCard(
                  ...
                  tempOk
                      ...
                      : PanelAgricultorStyles.warnText,
                  ranges: [
                    '${s.temperaturaSensorMin} °C',
                    '${s.temperaturaSensorMax} °C',
                  ],
                  optimo:
                      'Óptimo cultivo: ${s.temperaturaOptimaMin}–${s.temperaturaOptimaMax} °C',
                  device:
                      'Rango sensor: ${s.temperaturaSensorMin}–${s.temperaturaSensorMax} °C',
                ),
              ),
              buildSizedCard(
                _buildSensorCard(
                  ...
                  ranges: ['15 %', '100 %'],
                  optimo:
                      'Óptimo cultivo: ${s.humedadAireOptimaMin}–${s.humedadAireOptimaMax} %',
                  device: 'Rango sensor: 15–100 %',
                ),
              ),
              buildSizedCard(
                _buildSensorCard(
                  ...
                  sueloPct,
                  '#55a820',
                  sueloOk ? 'Normal' : 'Riego necesario',
                  sueloOk
                      ? PanelAgricultorStyles.sanoBg
                      : PanelAgricultorStyles.warnBg,
                  sueloOk
                      ...
                      : PanelAgricultorStyles.warnText,
                  ranges: ['10 %', '100 %'],
                  optimo: 'Mínimo recomendado: ${s.riegoMinimo} %',
                  device: 'Rango sensor: 10–100 %',
                ),
              ),
              buildSizedCard(
                _buildSensorCard(
                  'Intensidad de luz',
                  '${(s.intensidadLuzLux / 1000).round()}k',
                  'lux',
                  FontAwesomeIcons.sun,
                  luzPct,
                  '#b56c07',
                  luzOk ? 'Óptimo' : 'Fuera de rango',
                  luzOk
                      ? PanelAgricultorStyles.sanoBg
                      : PanelAgricultorStyles.warnBg,
                  luzOk
                      ? PanelAgricultorStyles.sanoText
                      : PanelAgricultorStyles.warnText,
                  ranges: ['0', '150 000 lux'],
                  optimo:
                      'Óptimo cultivo: ${(s.luzOptimaMin / 1000).round()}k–${(s.luzOptimaMax / 1000).round()}k lux',
                  device: 'Rango sensor: 0–150 000 lux',
                ),
              ),
              buildSizedCard(
                _buildSensorCard(
                  ...
                  ranges: ['0 %', '100 %'],
                  optimo:
                      'Óptimo cultivo: ${c.humedadHojaOptimaMin}–${c.humedadHojaOptimaMax} %',
                  device: 'Rango sensor: 0–100 %',
                  extraPillText: 'Estimado',
                  extraPillBg: PanelAgricultorStyles.estBg,
                  extraPillColor: PanelAgricultorStyles.estText,
                  note: 'Sin sensor físico · calculado por IA',
                ),
              ),
              buildSizedCard(
                _buildSensorCard(
                  ...
                  '#378ADD',
                  'Estimado',
                  PanelAgricultorStyles.estBg,
                  PanelAgricultorStyles.estText,
                  ranges: ['0', '5 m/s'],
                  optimo:
                      'Referencia invernadero: ${c.flujoAireRefMin}–${c.flujoAireRefMax} m/s',
                  device: 'Rango sensor: 0–5 m/s',
                  note: 'Sin sensor físico · calculado por IA',
                ),
              ),
            ],
  ...
  Widget _buildSensorCard(
    ...
    required String optimo,
    required String device,
    String? extraPillText,
    Color? extraPillBg,
    Color? extraPillColor,
    String? note,
  }) {
    return Container(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text(
                ranges.first,
                style: const TextStyle(fontSize: 9, color: Color(0xFF8FA69C)),
              ),
              Text(
                ranges.last,
                style: const TextStyle(fontSize: 9, color: Color(0xFF8FA69C)),
          ...
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  optimo,
                  style: const TextStyle(fontSize: 9, color: Color(0xFF854F0B)),
    ...
    return _buildSeccion(
      ...
      Wrap(
        ...
        children: [
          ...
          _buildMetricaCard(
            'Manchas',
            m.manchasDetectadas.toString(),
            '',
            FontAwesomeIcons.circleNodes,
            m.manchasDetectadas.toDouble(),
            [3, 10, 15],
            ['0', '3', '10', '15'],
            [
      ...
      nota:
          'Análisis realizado por visión artificial sobre la imagen capturada.',
    );
  }
  ...
  Widget _buildMetricaCard(
    ...
    dynamic icon,
    double valor,
    List<double> umbralesMax,
    List<String> escalaLabels,
    List<Map<String, String>> leyenda,
  ) {
        ...
        children: [
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final double dotLeftPx =
                  (trackWidth * (dotLeft / 100)) -
                  5.5; // -5.5 para centrar el punto de 11px
                ...
                children: [
                  Container(
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: Container(color: const Color(0xFFC0DD97)),
                        ),
                        Expanded(
                          child: Container(color: const Color(0xFFFAC775)),
                        ),
                        Expanded(
                          child: Container(color: const Color(0xFFF09595)),
                        ),
                      ],
                    ),
                  ),
        ...
        children: [
          ...
          if (!_dispositivoDesconectado)
            ...
            ElevatedButton.icon(
              ...
              icon: const FaIcon(FontAwesomeIcons.plug, size: 14),
              label: Text(
                _isReconnecting ? 'Reconectando...' : 'Reconectar Dispositivo',
              ),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.white,
          ...
          if (_errorReconexion.isNotEmpty)
            Padding(
              ...
              child: Text(
                _errorReconexion,
                style: const TextStyle(
                  color: PanelAgricultorStyles.critText,
                  fontSize: 13,
                  fontWeight: FontWeight.bold,
                ),
              ),
